...
public class RobotMove extends SendUDP{
	...
	for (int i = 0; i < commandByt.length; i++) {
	    byte[] e = robotMove.hexStringToByteArray(commandByt[i]);
	    commandbyteArray[i] = e[0];
	}
	byte[] changedCommandbyteArray = robotMove.swap(commandbyteArray);
	/*Debug purpose
	 ...
	 */
	// byte[] into int32
	int[] haha = robotMove.byteToint32(changedCommandbyteArray);
	for (int i : haha) {
